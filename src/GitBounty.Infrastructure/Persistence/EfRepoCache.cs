...
public sealed class EfRepoCache(
    IDbContextFactory<GitBountyDbContext> contextFactory,
    TimeProvider time) : IRepoCache
{
    ...
    static async Task<RepoCacheEntry> UpsertRepoAsync(
        GitBountyDbContext db, RepoCandidate repo, DateTimeOffset now, CancellationToken ct)
    {
    ...
    static async Task SaveIgnoringRacesAsync(GitBountyDbContext db, CancellationToken ct)
    {
        try
        {
